...
@Component({
  ...
  template: `
    <div class="fixed inset-0 z-[200] flex justify-end">
      ...
      <!-- Panel -->
      <div class="relative flex h-full w-full max-w-[340px] flex-col bg-[#111111] shadow-2xl transition-transform duration-300">
        <!-- Header -->
        <div class="flex items-center justify-between border-b border-white/10 px-6 py-5">
          <div class="flex items-center gap-3">
            <div class="flex h-8 w-8 items-center justify-center rounded-lg bg-white/5 text-white/80">
        ...
        <div class="flex items-center justify-between px-6 py-3 border-b border-white/5 bg-white/[0.02]">
          <span class="text-xs font-semibold text-white/50">{{ notifications().length }} Mensajes</span>
          <button 
        ...
        <div class="flex-1 overflow-y-auto no-scrollbar p-4">
          @if (notifications().length === 0) {
            <div class="flex h-40 flex-col items-center justify-center text-center">
              <span class="material-symbols-rounded mb-3 text-4xl text-white/10">notifications_paused</span>
            ...
            <div class="flex flex-col gap-3">
              @for (notif of notifications(); track notif.id) {
                ...
                >
                  <div class="mt-1 flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-blue-500/10 text-blue-400">
                  ...
                  <div class="flex min-w-0 flex-1 flex-col">
                    <div class="flex items-center justify-between gap-2">
                      ...
                      <span class="shrink-0 text-[10px] font-bold text-white/30">{{ notif.timestamp | date:'shortTime' }}</span>
                    </div>
