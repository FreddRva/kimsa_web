...
export class CustomerFacade {
  ...
  private loadCustomers() {
    ...
    this.repository.getCustomers().subscribe({
      next: (data) => { this.customers.set(data); this.loading.set(false); this.checkBirthdays(data); },
  ...
  private checkBirthdays(list: Customer[]) {
    ...
    list.filter(c => {
      ...
      return month === today.getMonth() + 1 && day >= today.getDate() && day <= today.getDate() + 3;
    }).forEach(c => {
  ...
  async saveCustomerWithDateFormat(customerData: Partial<Customer> & { id?: string }): Promise<void> {
