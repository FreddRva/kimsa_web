...
export class ProductFacade {
  ...
  async saveProduct(productData: Partial<Product> & { id?: string }, isEditing: boolean): Promise<void> {
  ...
  async saveStationDistribution(changes: { id: string; station: string; components: { name: string; station: string }[] }[]): Promise<void> {
    await Promise.all(changes.map(c => this.repository.updateProduct(c.id, { station: c.station, components: c.components })));
  }
